				...
				{#if currentRecord}
					<div class="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
						<div class="flex items-center justify-between">
							<div>
								<h3 class="font-medium text-gray-900">{currentRecord.title_en || currentRecord.law_name}</h3>
					...
					<div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
						...
						{#if parseResult.has_errors}
							<div class="mt-3 rounded-md bg-amber-50 border border-amber-200 p-3">
								<p class="text-sm text-amber-700">Parse completed with errors. Partial data may be available.</p>
							</div>
						{/if}
					</div>
				{/if}
