...
# Update UK LRT old_style_number column from Airtable CSV export
#
# Usage:
#   cd backend && mix run ../scripts/data/update_uk_lrt_old_style_number.exs [csv_path] [--dry-run]
#
# Examples:
#   mix run ../scripts/data/update_uk_lrt_old_style_number.exs ~/Documents/Airtable_Exports/UK-EXPORT.csv
#   mix run ../scripts/data/update_uk_lrt_old_style_number.exs ~/Documents/Airtable_Exports/UK-EXPORT.csv --dry-run
...
defmodule UkLrtOldStyleNumberUpdater do
  @moduledoc """
  Updates UK LRT old_style_number column from Airtable CSV export.
  """
  ...
  def run(csv_path, opts \\ []) do
    ...
    IO.puts("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    IO.puts("  UK LRT old_style_number Update from Airtable CSV")
    IO.puts("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    {name_idx, old_style_idx} = read_headers(csv_path)
    IO.puts("  Name column index: #{name_idx}")
    IO.puts("  old_style_number column index: #{old_style_idx}\n")
    ...
    result =
      ...
      |> Enum.reduce(%{updated: 0, skipped: 0, not_found: 0, no_data: 0, errors: []}, fn row,
                                                                                         acc ->
        process_row(row, name_idx, old_style_idx, name_to_id, dry_run, acc)
      end)
  ...
  defp read_headers(csv_path) do
    ...
    old_style_idx = Enum.find_index(headers, &(&1 == "old_style_number"))
    ...
    unless old_style_idx do
      IO.puts("  ✗ old_style_number column not found in CSV")
      System.halt(1)
    end

    {name_idx, old_style_idx}
  end
  ...
  defp process_row(row, name_idx, old_style_idx, name_to_id, dry_run, acc) do
    name = get_field(row, name_idx)
    old_style = get_field(row, old_style_idx)
    ...
    cond do
      ...
      true ->
        ...
        if dry_run do
          ...
          case update_record(id, old_style) do
            :ok ->
              %{acc | updated: acc.updated + 1}
  ...
  defp update_record(id, old_style) do
    import Ecto.Query

    query =
      from(u in "uk_lrt",
        where: u.id == ^id,
        update: [set: [old_style_number: ^old_style]]
      )
  ...
  defp print_summary(result, duration_sec, dry_run) do
    ...
    IO.puts("  No data:      #{result.no_data} (empty old_style_number)")
  ...
  case args do
    ...
    [] ->
      ...
      if File.exists?(default_path) do
        ...
        IO.puts(
          "Usage: mix run ../scripts/data/update_uk_lrt_old_style_number.exs <csv_path> [--dry-run]"
        )
...
# Run the updater
UkLrtOldStyleNumberUpdater.run(csv_path, update_opts)
