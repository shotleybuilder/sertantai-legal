defmodule SertantaiLegal.Repo.Migrations.DropLegacyFitnessColumns do
  @moduledoc """
  Drop legacy P-dimension fitness columns from legal_register.
  Replaced by v0.3 fitness_entities + compiled_applicability.
  Simplifies has_fitness to only check fitness_entities.
  Requires full uk_lrt view + trigger rebuild.
  """
  use Ecto.Migration
  ...
  def up do
    ...
    # 2. Drop has_fitness FIRST (depends on fitness_person)
    ...
    # 4. Recreate has_fitness with simplified expression

    execute("""
    ALTER TABLE legal_register ADD COLUMN has_fitness boolean
    GENERATED ALWAYS AS (
      fitness_entities IS NOT NULL
    ) STORED
    """)

    # 4. Recreate view (without legacy columns)
    execute("""
    ...
    BEGIN
      UPDATE legal_register SET
        ...
        compiled_applicability = NEW.compiled_applicability
      WHERE id = OLD.id;
      RETURN NEW;
    END;
    $function$ LANGUAGE plpgsql
    ...
    # 7. Recreate DELETE trigger (unchanged)
    execute("""
  ...
  def down do
    # Reverse would require re-adding columns and rebuilding view — omitted
  end
end
