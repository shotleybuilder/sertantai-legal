defmodule SertantaiLegal.Repo.Migrations.AddExplanatoryNote do
  @moduledoc """
  Add explanatory_note text column to legal_register.

  Must drop and recreate uk_lrt view + INSTEAD OF triggers because the view
  depends on the underlying table columns.
  """
  ...
  def up do
    ...
    alter table(:legal_register) do
      add :explanatory_note, :text
    end

    # Recreate view with explanatory_note after significance_parts
    execute("""
  ...
  def down do
    ...
    # Remove the column
    alter table(:legal_register) do
      remove :explanatory_note
    end

    # Recreate view WITHOUT explanatory_note (restores previous state)
    execute("""
    CREATE VIEW uk_lrt AS
    SELECT
      id, family, family_ii, name, title_en, year, number, acronym, old_style_number,
    ...
    # Recreate INSTEAD OF INSERT trigger function (without explanatory_note)
    execute("""
    CREATE OR REPLACE FUNCTION uk_lrt_view_insert() RETURNS trigger AS $function$
    BEGIN
      INSERT INTO legal_register (
    ...
    # Recreate INSTEAD OF UPDATE trigger function (without explanatory_note)
    execute("""
    CREATE OR REPLACE FUNCTION uk_lrt_view_update() RETURNS trigger AS $function$
    BEGIN
      UPDATE legal_register SET
