defmodule SertantaiLegal.Repo.Migrations.FixUkLrtViewWithTriggers do
  @moduledoc """
  Recreate the uk_lrt view properly with INSTEAD OF triggers.

  Migration 20260702000003 broke the view by using `SELECT * FROM legal_register_uk`
  which lost the INSTEAD OF INSERT/UPDATE/DELETE triggers and the `source_url AS
  leg_gov_uk_url` alias. This caused 59 test failures because PostgreSQL's BEFORE
  triggers on partitioned tables conflict with view inserts that route through
  partition logic.

  The original view (from 20260518000001) was:
  - SELECT with explicit columns from `legal_register WHERE country = 'uk'`
  - Excluded `country` and `jurisdiction` columns
  - Aliased `source_url` as `leg_gov_uk_url`
  - Had INSTEAD OF triggers that inserted directly into `legal_register`

  This migration restores that pattern with significance columns added.
  """
  ...
  def up do
    ...
    # Recreate with explicit columns (excluding country/jurisdiction, aliasing source_url)
    execute("""
    ...
    # Recreate INSTEAD OF INSERT trigger function (with significance columns)
    execute("""
    ...
    # Recreate INSTEAD OF UPDATE trigger function (with significance columns)
    execute("""
    ...
    BEGIN
      UPDATE legal_register SET
        ...
        making_review = NEW.making_review,
        making_review_at = NEW.making_review_at,
    ...
    # Recreate INSTEAD OF DELETE trigger function (unchanged)
    execute("""
