defmodule SertantaiLegal.Repo.Migrations.AddMakingFunnelView do
  @moduledoc """
  `making_funnel`: one row per law showing where it is in the Making funnel,
  the evidence behind `is_making`, and the next action (QQ-01a).

  Stages, in order of precedence:
  reviewed → enriched → lat_parsed → cleaned → skipped → deferred → triaged →
  detected → unclassified. `cleaned` also covers LAT session records left at
  `confirmed` after their LAT was deleted (before the `cleaned` status existed).

  `conflict` is true when a triage classification disagrees with the resolved
  `is_making`. The resolver keeps the higher tier; the flag keeps it visible.

  Read-only view over `legal_register` and `scrape_session_records`; no
  triggers, not part of the `uk_lrt` view.
  """
  ...
  def down do
    execute("DROP VIEW IF EXISTS making_funnel")
  end
end
