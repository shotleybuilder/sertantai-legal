...
| `sort_key` | Utf8 | no | **Composite hierarchical sort key** â€” machine-sortable string encoding the full structural path. `ORDER BY sort_key` recovers correct document order within a law. Format: `{schedule}.{part}.{chapter}.{heading}.{provision}.{sub}.{paragraph}.{sub_paragraph}~{extent}` where each segment is a normalised 3-digit provision key (see encoding rules). Schedule segment uses “S” prefix to sort after body content. See design note below. |
...
> **Design note â€” three-column identity**: The `section_id` is a structural citation derived from parliament’s own canonical addressing scheme. “Section 41A of the Environment Act 1995” never changes â€” even when further amendments insert 41B, 41C, or 41ZA. This is stable across amendments, unlike an integer position which requires renumbering when sections are inserted. The `sort_key` encodes the **full structural hierarchy** as a composite key â€” each ancestor level (schedule, part, chapter, heading, provision, sub, paragraph, sub_paragraph) is normalised via `normalize_provision_to_sort_key` into a zero-padded segment. Example: `Pt.3 > Ch.1 > s.12(1)(a)` encodes as `000.003.000.000.001.000.000.000.000.000.012.000.000.001.000.000.010.000.000.000.000.000~`. This ensures paragraphs from different sections never collide and structural rows sort correctly relative to their children. The `position` column remains as a convenience integer for fast range scans.
>
