...
| `fitness_applies_count` | `Int32` | Mentions with AppliesTo polarity |
| `fitness_disapplies_count` | `Int32` | Mentions with DisappliesTo polarity |
| `compiled_applicability` | `Utf8` | JSON expression tree for rules engine evaluation. See "Compiled Applicability Trees" section below. |
...
The tree is a tagged union (`"op"` field discriminates). Six node types:

```json
// Leaf: does the customer match any of these codes in this dimension?
{ "op": "Match", "dimension": "personal", "codes": ["employer", "contractor"], "match_op": "AnyOf" }
...
// Child must NOT match (DisappliesTo)
{ "op": "Not", "child": { ... } }

// Match `then` only if `condition` matches
{ "op": "Conditional", "condition": { ... }, "then": { ... } }

// Temporal: law applies between from and to (null = unbounded)
{ "op": "TimeWindow", "from": "2025-10-01", "to": null }
```

### Scope Dimensions

The `dimension` field on `Match` nodes uses the four legal scope dimensions:

| Dimension | What it answers | Example codes |
|-----------|----------------|---------------|
| `personal` | Who does this apply to? | `employer`, `public_authority`, `any_person` |
| `material` | What activity/subject? | `construction_work`, `wild_bird`, `marine_conservation_zone` |
| `territorial` | Where? | `england`, `scotland`, `uk_marine_licensing_area` |
| `conditional` | What qualifying criteria? | `at_work`, `5_or_more_employees` |

### Example: CDM-like Regulations

"These Regulations apply to every employer and contractor doing construction work, except domestic premises."
...
```json
{
  ...
  "children": [
    { "op": "Match", "dimension": "personal", "codes": ["employer", "contractor"], "match_op": "AnyOf" },
    { "op": "Match", "dimension": "material", "codes": ["construction_work"], "match_op": "AnyOf" },
    { "op": "Not", "child": {
      "op": "Match", "dimension": "material", "codes": ["domestic_premises"], "match_op": "AnyOf"
    }},
    { "op": "TimeWindow", "from": "2015-04-06", "to": null }
  ]
}
```
...
Recursive tree walk against a customer profile. See `docs/controls/FITNESS-APPLICABILITY.md` for Elixir evaluator code (~50 lines).

### Coverage

- 701 laws currently have compiled trees
- Laws without fitness mentions have `compiled_applicability = NULL`
- Tree size: typically 200 bytes – 5 KB JSON per law
